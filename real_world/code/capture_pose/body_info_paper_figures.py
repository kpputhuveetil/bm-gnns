...
def draw_line(event):
    ...
    if canvas.index < len(canvas.limbs):
        if str(event.type) == 'ButtonPress':
            ...
            create_circle(event.x, event.y, 10, canvas)

        elif str(event.type) == 'Motion':
            x, y = event.x, event.y
            x1, y1 = canvas.old_coords
            canvas.delete(canvas.old_line_id)
            canvas.old_line_id = canvas.create_line(x, y, x1, y1, fill='red', width=8)
...
if __name__ == "__main__":
    ...
    bg= ImageTk.PhotoImage(file=osp.join(save_dir, 'pose_est_uncov.png'))
