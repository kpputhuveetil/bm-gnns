...
#%%
def gnn_cma(env_name, idx, model, device, target_limb_code, iter_data_dir):
    ...
    if abs(best_reward - env_reward) > 15:
        save_data_to_pickle(
            idx, 
            seed, 
            best_action, 
...
def evaluate_dyn_model(target_limb_code, trials, model, iter_data_dir, device, num_processes):
    ...
    for j in tqdm(range(math.ceil(trials/num_processes))):
        with multiprocessing.Pool(processes=num_processes) as pool:
            for i in range(num_processes):
                ...
                result = pool.apply_async(gnn_cma, args = (env_name, idx, model, device, target_limb_code, iter_data_dir), callback=counter_callback)
...
if __name__ == '__main__':
    ...
    data_dir = osp.join(checkpoint, 'one_step_improvement')
    ...
    for iter in tqdm(range(iterations)):
        ...
        # gnn_train_test.set_new_save_dir(data_dir)
        # tracking_file.write(f"----Iteration {iter}----\n")
        # tracking_file.write(f"Checkpoint: {checkpoint}\n")
        # tracking_file.flush()

        pred_sim_reward_error = evaluate_dyn_model(target_limb_code, trials, gnn_train_test.model, data_dir, device, num_processes)

        print(f"Eval Model Iteration {iter} - Ave Pred-Sim Error {np.mean(pred_sim_reward_error)}")
        # print(pred_sim_reward_error)
        # tracking_file.write(f"Eval Model Iteration {iter} - Ave Pred-Sim Error {np.mean(pred_sim_reward_error)}\n")
        # tracking_file.flush()
        # print(seeds)
